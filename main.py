...
server = None
g_mail_username = "itsthesantabot@gmail.com"
g_mail_password = os.getenv("G-MAIL-PASSWORD")
link = "https://github.com/SantaBot/"
send_from = g_mail_username
to = get_config_lines(4).split(",").len()
...
def start():
    try:
        if couldGetArea4:
            print(area4.div15(), "\n" + configLines[12] + "\n" + area4.div15())
        server = smtplib.SMTP(get_config_lines(10), 587)
        server.ehlo()
        server.starttls()
        server.login(g_mail_username, g_mail_password)
        server.sendmail(send_from, to, format_message())
        server.close()
    except:
